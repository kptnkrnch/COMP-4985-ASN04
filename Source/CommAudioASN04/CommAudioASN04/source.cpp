...
bool GetSongList(std::string songfile, HWND lbHwnd);
                          ...
                          WPARAM wParam, LPARAM lParam)
{
	...
	switch (Message)
	{
		...
		case WM_CREATE:
			...
			if (!GetSongList(songlist, listbox)) {
				MessageBox(hwnd, TEXT("Error, need song-list.txt!"), TEXT("ERROR"), MB_OK);
				exit(1);
			}
		...
		case WM_PAINT: //prints statistics
			...
			if (clientStarted) {
				TextOut(hdc, 10, 200, TEXT("Client Started"), strlen("Client Started"));
			}
			if (serverStarted) {
				TextOut(hdc, 10, 200, TEXT("Server Started"), strlen("Server Started"));
			}

			if (MicFlag && (clientStarted || serverStarted)) {
				TextOut(hdc, 10, 250, TEXT("Streaming Microphone"), strlen("Streaming Microphone"));
			}
			if (multicastFlag && mode == MODE_SERVER && serverStarted) {
				TextOut(hdc, 10, 300, TEXT("Streaming Multicast"), strlen("Streaming Multicast"));
			}
			if (multicastFlag && mode == MODE_CLIENT && clientStarted) {
				TextOut(hdc, 10, 300, TEXT("Receiving Multicast"), strlen("Receiving Multicast"));
			}
...
--
-- RETURNS: bool - false if it failed to open the song list 
--
-- NOTES:
